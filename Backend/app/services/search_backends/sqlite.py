...
class SQLiteSearchBackend(SearchBackend):
    ...
    def _score_rows(
        self, query_embedding: List[float], rows: list
    ...
    @staticmethod
    def _recency(created_at: Optional[str]) -> float:
    ...
    def _composite_score(
        self, cosine: float, row: dict, max_usage: int
    ) -> float:
    ...
    def search(
        ...
        with session() as conn:
            ...
            if query_embedding:
                ...
                qualifying = [(cosine, row) for cosine, row in scored if cosine >= similarity_threshold]
                ...
                if qualifying:
                    ...
                    max_usage = max(
                        (r["usage_count"] or 0 for _, r in qualifying), default=0
                    )
                    ranked = [
